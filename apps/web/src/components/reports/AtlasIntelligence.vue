    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div class="space-y-4">
        ...
        <div
          ...
          class="rounded-atlas-lg border border-atlas-border bg-atlas-surface p-5"
        >
          <h3 class="text-sm font-semibold text-atlas-text-primary mb-3">
            Attention
          </h3>
          <ul class="space-y-3">
            <li
            ...
            >
              <span
                ...
                :class="item.level === 'high' ? 'bg-atlas-error' : 'bg-atlas-warning'"
                :aria-label="levelLabel(item.level) + ' attention'"
              />
              <div class="min-w-0 flex-1">
                <p class="text-sm text-atlas-text-primary">
                  {{ item.text }}
                </p>
                <p class="mt-0.5 text-xs text-atlas-text-muted tabular-nums">
        ...
        >
          ...
          <ul class="space-y-3">
            ...
            >
              <span
                class="mt-0.5 inline-block size-2 rounded-full shrink-0 bg-atlas-brand"
                aria-hidden="true"
              />
              <div class="min-w-0 flex-1">
                <p class="text-sm text-atlas-text-primary">
        ...
        <!-- Analysis status -->
        <div class="rounded-atlas-lg border border-atlas-border bg-atlas-surface p-5">
          <h3 class="text-sm font-semibold text-atlas-text-primary mb-2">
            Analysis coverage
          </h3>
          <div class="flex items-center gap-3">
            <div class="flex-1 h-2 rounded-full bg-atlas-surface-muted overflow-hidden">
              <div
                class="h-full rounded-full bg-atlas-brand transition-all"
            ...
            <span class="text-xs text-atlas-text-muted tabular-nums whitespace-nowrap">
          ...
          <p class="mt-2 text-xs text-atlas-text-muted">
            ...
            <template v-else>
              ...
              <RouterLink
                to="/app/tickets"
                class="atlas-focus-ring text-atlas-brand hover:text-atlas-brand-hover transition-colors"
              >
                View tickets →
              </RouterLink>
            </template>
      ...
      <div class="space-y-4">
        ...
        >
          ...
          <ul class="space-y-2.5">
            ...
            >
              ...
              <span class="text-xs text-atlas-text-muted tabular-nums">
        ...
        <div class="rounded-atlas-lg border border-atlas-border bg-atlas-surface p-5">
          ...
          <p class="text-xs text-atlas-text-muted mb-3">
            Identified in analyzed conversations
          </p>
          <div class="space-y-2">
            <div class="flex items-center justify-between">
              <span class="text-sm text-atlas-text-primary">Feature requests</span>
              <span class="text-sm font-medium text-atlas-text-primary tabular-nums">{{ insights.signals.featureRequests }}</span>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-sm text-atlas-text-primary">Bug reports</span>
              <span class="text-sm font-medium text-atlas-text-primary tabular-nums">{{ insights.signals.bugReports }}</span>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-sm text-atlas-text-primary">Knowledge gaps</span>
              <span class="text-sm font-medium text-atlas-text-primary tabular-nums">{{ insights.signals.knowledgeGaps }}</span>
            </div>
          </div>
          <div class="mt-3 pt-3 border-t border-atlas-border flex items-center justify-between">
            <span class="text-sm font-medium text-atlas-text-primary">Total signals</span>
            <span class="text-sm font-semibold text-atlas-text-primary tabular-nums">{{ insights.signals.totalSignals }}</span>
          </div>
        </div>
      </div>
    </div>
